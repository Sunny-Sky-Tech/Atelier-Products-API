...
const getProducts = (request, response) => {
  ...
  pool.query("SELECT * FROM products WHERE id > 0 AND id < 6", (error, results) => {
    if (error) {
      console.log(error);
    }
...
const getProductInfo = (request, response) => {
  ...
  var queryFeatures = "SELECT feature, value FROM features WHERE product_id = $1";
...
const getProductStyles = (request, response) => {
  ...
  var stylesQuery = "SELECT id, name, original_price, sale_price, default_ FROM styles WHERE product_id = $1";
